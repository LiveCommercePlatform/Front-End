...
export const useLiveChat = (roomId: string, currentUserId?: string) => {
  ...
  }, [roomId, currentUserId, setupCookie, flushQueue, scheduleReconnect, markAuthRequired]);
  ...
  const sendMessage = useCallback(
    ...
    [authRequired, currentUserId]
  );

  const sendPending = messages.filter((m) => m.pending).length;
  const canSend = !!currentUserId && !authRequired && status === "open";
