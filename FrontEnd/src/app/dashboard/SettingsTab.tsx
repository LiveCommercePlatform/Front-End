...
import { Profile, ProfileFormValues, PasswordFormValues } from "@/types";
...
export default function SettingsTab({ profile }: { profile: Profile }) {
  ...
  const isValidPotalCode = (postal_code: string) => /^\d{10}$/.test(postal_code);
  ...
  if (data.postal_code && !isValidPotalCode(data.postal_code)) {
    toast.error("کد پستی باید 10 رقم داشته باشد!");
    return;
  }
